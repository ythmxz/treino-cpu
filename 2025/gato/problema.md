...
Cada caso de teste constituído de duas linhas, a primeira linha contém três números inteiros $H$, $V$ e $R$ (−50 <= $H$, $V$ <= 50 e 0 < $R$ <= 50) representando a localização leste-oeste (H) e sul-norte (V) da casa que Guilhermina e Gatito estão, e o raio (R) da casa. Na segunda linha são dois números inteiros $G_h$ e $G_v$ (−100 <= $G_h$, $G_v$ <= 100), a posição de Gatito com $G_h$ a coordenada leste-oeste e $G_v$ a coordenada sul-norte.
